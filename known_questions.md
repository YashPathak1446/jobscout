...
the gate itself, which makes it the more serious of the two found that day.

`two_degrees` on `gemini` **failed and then passed, on identical code**, minutes
apart:

    run 1   FAIL: no resume passed validation. All 1 were quarantined.
            ... bullet 1: 175 chars falls in the line-2 orphan zone
    run 2   1 valid, best job 43.5%   PASS

And against the same rung on 2026-08-27, before any of this: `6 (+4 held), 5,
4 valid`. Today: `3, 5, 0`. Same code, same frozen corpus.

### Why

Two live model calls per row, neither cached.

1. **Extraction.** `complete_json`'s gemini branch calls
   `client.models.generate_content` directly. There is no cache on that path at
   all, so the `.tex` under test is re-generated by a non-deterministic model on
   every run — a different resume each time, before the pipeline starts.
2. **Generation.** The harness passes `use_cache=False` deliberately, so bullet
   rewriting is fresh too.

The `none` row is reproducible only because it calls no model. So the property
holds exactly where it is least needed and fails on the rung that is the paid
product.

### Why it matters more than the row it was found on

The plan's premise is that *"done cannot mean no bugs left. It has to mean a
fixed checklist passes."* **A checklist that flips between PASS and FAIL on
identical code is not fixed**, and it fails in the worse direction too: rule 3
makes an acceptance failure at any phase exit automatically in scope, so a
flaky gate spends Phase 1 and Phase 2 generating false alarms — and, the other
way round, a green run is not evidence the phase did no harm.

This is the R81 lesson pointed at the harness rather than at a model: **check
the instrument before trusting the reading.** The first run's failure was
nearly written up as a regression in this change.

### The options, none chosen

1. **Cache extraction, keyed on file bytes + rung + model.** The cache-key
   discipline of R11/R45/R80 already says what the key must carry. Makes the
   import half reproducible and cuts the run's cost and time. Does not touch
   generation.
2. **Re-run a failing row once before reporting it**, and report a row as failed
   only if it fails twice. Cheap, honest about what it is, and it hides genuine
   intermittent faults — which for a product that writes one resume per posting
   may be exactly the faults that matter.
3. **Pin what can be pinned and state the rest.** Accept that a model rung is a
   sample, report the row as a measurement with a date rather than a gate, and
   let only `none` gate. This is the honest version and it weakens the bar.
4. **Seed or temperature-zero the extraction call.** Reduces variance without
   removing it; Gemini does not promise determinism.
...
`heuristic_schema` returns `experiences: []` **by design**, keeping the
section's raw lines under `_unparsed` for a person to correct — 17 lines for
`two_degrees`, 29 for `glued_runs`. `tex_renderer` does not render `_unparsed`,
so no `\section{Experience}` is emitted, and `parse_latex_resume` finds nothing
to tailor.

**Rendering `_unparsed` as one catch-all experience does not close this.**
`_validate_experiences` requires `id`, `title`, `company` and two bullets on
every entry; a catch-all block has no title and no company, so it lands in
`needs_review` and the "at least one valid" assertion fails anyway. Only
genuine role-splitting closes it, and that is the judgement the floor exists
to refuse to guess at — the pattern reader never invents content (`A WS` for
AWS, `Lakeside UniversityFairview, IL`).

Even past that guard the score bar is the next wall: with no experiences and no
projects, `resume_terms` reads no keywords, `keyword_overlap` is empty, and the
blend collapses to `embedding_pct × 0.7`, so a skills-only match needs above
~57 to clear 40.

### What it bears on

- **R81 makes `none` the hosted free tier.** Ollama is closed for hosted on
  cost, so the free tier is bring-your-own-key or this floor.
- **`CLAUDE.md`'s commercial pitch is "pay us, or run it free yourself".** The
  free half is what makes charging honest to an audience of unemployed job
  seekers.
- **The plan's opening claim** — *"a stranger with no API key can put a PDF in
  and get a sendable one-page PDF out"* — **is not currently evidenced.** It
  was believed on the strength of an acceptance run that was importing with
  Gemini while reporting `none`.

Note what still works with no key: discovery (R34), scoring and selection
(R36), and a `.tex` master resume end to end. The gap is specifically **PDF or
DOCX import with no model**, which is the path a stranger arrives on.

### The options, none chosen

1. Commit `.tex` counterparts for the two fixtures, so the `none` row measures
   the free-tier *pipeline* and free-tier *import* becomes a named gap. Cheap,
   and it must be written into the file in as many words or it is a quieter
   version of R83.
2. Make the floor split roles well enough to produce a runnable resume. A
   project, and it argues with the pattern reader's whole design.
3. Accept it and say so in `README.md`: without a model, bring a `.tex`.
4. Let it decide the hosted free tier — if the floor cannot serve a stranger,
   the free tier is bring-your-own-key and the pitch changes wording.
...
## R83. The acceptance run imported through a rung it was not asked for

**Decision:** (2026-08-27) `scripts/acceptance.py` now pins the same rung for
the resume import as for the pipeline, and reports which rung read the resume.
The `none` row fails as a result, and **that failure stands**: it is the gate
describing the product accurately for the first time. Phase 0's exit criterion
is unmet again, deliberately.

### The docstring was right and the wiring was not

`complete_json` has taken a `backend` argument since R80, and its docstring
names this exact bug as the reason:

> import and generation are the project's two model consumers, and if one
> obeyed a `--backend` flag while the other went on detecting, a run pinned to
> Ollama would have imported through Gemini and nothing would have said so.

**No caller ever passed it.** The only production route in is
`init_profile.extract_resume`, which reaches `complete_json` through
`to_schema` — and `to_schema` invokes its agent with one positional argument,
so `backend` took its default and the call auto-detected. The parameter was
written to carry a pin and was computed and never read.

That is the recurring bug of this codebase for the seventh time, and R55's
shape again: a correct comment crediting a guard that never fires. The comment
was not stale. It was never true of anything but itself.

### It was not hypothetical, and the evidence was on disk

`%TEMP%\jobscout-acceptance\acceptance_two_degrees.tex`, written by the run
labelled `none`, contained:

    \section{Education}  \section{Experience}  \section{Projects}  \section{Technical Skills}

with six `\resumeSubheading` entries. `heuristic_schema` returns
`experiences: []` **by design** — splitting roles apart is the judgement a
regex cannot make — so the floor cannot produce that file. Gemini wrote it, on
the row documented as *"the rung a stranger with no key lands on"*. After the
fix the same file is `\section{Education}` and `\section{Technical Skills}`,
three subheadings, all of them education.

`test_backend_selection.py` had held the two consumers together since R80 and
could not see this, because it checks that both consumers **resolve** the same
way, not that anybody ever **tells** them. A test that checks one path against
itself proves nothing about the other; this was a third hop neither path
walked.

### The fix

`llm_backends.effective_backend(backend, profile, gemini_key)` is the
`resolve_backend`-then-`detect` pair, in one place. It **removes** a duplicate
rather than adding a resolution site: `resolve_backend` is still called exactly
once in `llm_backends.py`, so `test_both_call_the_one_resolver`'s three-file
map is unchanged and `init_profile` is a consumer of the resolver rather than a
fourth implementation of it.

`extract_resume(file_bytes, filename, backend=None)` resolves once and binds
the answer with `functools.partial`, so `to_schema` still calls its agent with
one positional argument and the one-argument contract in three test modules
holds by construction rather than by care. It returns `rung`, in **three
states**: a rung name, or `None` meaning no model was consulted and none was
needed — which is not `"none"`, the rung a person chose.

Import provenance is deliberately **not** in `state["backend"]`. The
orchestrator does not do the import; the resume reaches it as a `.tex` path,
and for Priya the import happens in a different call from the run that consumes
the file. A claim there would be a record of an assertion rather than a record
of what happened, which is the distinction `_rung_used` exists to hold.

### Two tests that could not fail, found by making them

`test_nothing_detects_when_a_rung_is_pinned` was first written to patch
`detect` with something that raises. It passed on the broken code. `to_schema`
wraps its agent call in `except Exception`, and `AssertionError` is an
`Exception`, so the detector's objection was swallowed by the very handler the
call runs through. It now counts calls instead of raising.

Making it fail on purpose then printed **a live Gemini API key** into the test
log, because the watcher recorded `detect`'s kwargs and `gemini_key` is one of
them. It records a bare flag now. Both were found by reverting the fix and
reading what the suite actually said — the instrument checked before the
reading was trusted (R81), applied to a test rather than a measurement.

### What it costs

`none` now fails for `two_degrees` and `glued_runs`, at
`_refuse_an_empty_resume`, because the floor cannot produce a resume the
pipeline will run — logged as **Q26**. Priya passes and always did, for a
reason the report now states: her profile is not rebuilt, so her import is
written and thrown away. `gemini` is unaffected.

This sits inside rule 3 of the plan — *regression is in scope; discovery is
not.* The harness was under-testing the gate every later phase leans on, and
Phase 1's exit criterion is this same run against a deployed instance.

### Verified

1016 tests, three baselines. Both new tests fail on the reverted fix, checked.
`--rung none` fails 2 of 3 with the expected `ValueError`, and the temp `.tex`
lost its `\section{Experience}`.

**`--rung gemini` is unchanged in what it calls, and that had to be argued
rather than observed**, because the run is not reproducible (Q27). Before the
fix the import resolved `None -> "auto" -> detect() -> "gemini"`; after, it is
pinned to `"gemini"`. `detect()` returns `gemini` on this machine — checked —
