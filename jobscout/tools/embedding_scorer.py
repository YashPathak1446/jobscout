...
def embed_resume_components(parsed_resume) -> dict[str, list[float]]:
    ...
    for exp in parsed_resume.experiences:
        text = f"{exp.title} {exp.organization} {' '.join(exp.bullets)}"
    ...
    for proj in parsed_resume.projects:
        text = f"{proj.title} {proj.tech_line} {' '.join(proj.bullets)}"
        vec = _get_embedding(text, "RETRIEVAL_DOCUMENT")
        if vec:
    ...
    if parsed_resume.skills_text:
        vec = _get_embedding(parsed_resume.skills_text, "RETRIEVAL_DOCUMENT")
        if vec:
...
def embed_resume_components_mock(parsed_resume) -> dict[str, list[float]]:
    ...
    embeddings = {}
    for exp in parsed_resume.experiences:
        text = f"{exp.title} {exp.organization} {' '.join(exp.keywords)} {' '.join(exp.bullets)}"
        embeddings[exp.id] = _mock_embedding(text)
    for proj in parsed_resume.projects:
        text = f"{proj.title} {proj.tech_line} {' '.join(proj.keywords)} {' '.join(proj.bullets)}"
        embeddings[proj.id] = _mock_embedding(text)
    if parsed_resume.skills_text:
        embeddings["__skills__"] = _mock_embedding(parsed_resume.skills_text)
